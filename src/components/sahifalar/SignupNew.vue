 ...
 export default {
   ...
   methods: {
     ...
     async handleSmsRequest(isResend = false) {
       ...
       // Agar SMS allaqachon yuborilgan bo‘lsa va bu resend emas bo‘lsa –
       // asosiy tugma "Tasdiqlash / Ro‘yxatdan o‘tish" uchun
       if (this.smsSent && !isResend) {
         this.handleSubmit();
         return;
       }
 
       // SMSni qayta-qayta so'rash limitini tekshiramiz
       if (this.attemptCount >= this.maxAttempts) {
         this.showPopup(
       ...
       // Faqat birinchi marta reCAPTCHA talab qilinadi
       if (isFirstRequest) {
       ...
       try {
         ...
         this.smsSent = true;
         this.smsCode = ""; // eski kodni tozalaymiz
         this.attemptCount += 1;
         this.startTimer();
         ...
         // Faqat birinchi reCAPTCHAni reset qilamiz
