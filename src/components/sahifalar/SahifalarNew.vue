<template>
  ...
  export default {
    data() {
      return {
        serviceName: '',  // name parametri
        serviceId: null,  // id parametri (yashirin)
        serviceImage: '',
        serviceTitle: '',
        serviceDescription: '',
        serviceText: '',
    ...
    methods: {
      async fetchServiceData() {
        // URL parametridan name-ni olish
        this.serviceName = this.$route.params.name;
  
        // Query parametridan id-ni olish
        ...
        try {
          // API orqali id bilan xizmatni olish
          const response = await axios.get(
            `https://new.pochta.uz/api/v1/public/futer-menu-items/${this.serviceId}/`
          );
          const serviceData = response.data; // API dan qaytgan ma'lumotni olish
  
          if (serviceData) {
            this.serviceImage = this.convertToHttps(serviceData.save_image);
            this.serviceTitle = serviceData.name_uz || 'Xizmat nomi mavjud emas';
            this.serviceDescription = serviceData.description_uz || 'Xizmat tavsifi mavjud emas';
            this.serviceText = serviceData.text_uz || '';
          } else {
            console.error("Xizmat topilmadi!");
          }
        } catch (error) {
